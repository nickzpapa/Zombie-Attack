...
public class sand3 implements Screen {
	...
	sand3() {		
		...
		ZombieAttack.money = 300;



		noClickZone = new NoClickZone("NoNoZones/level9.nono");		
		towerList = new TowerList(stage, zombieList, noClickZone);		

	
		tiledMap = new TmxMapLoader().load("Maps/Level9.tmx");
	...
	public void render(float delta) {
		...
		if(zombieList.isLevelFinished() && zombieList1.isLevelFinished()){
			musicManager.stop();
			optionsPreferences.setCurrentLevel(3);
			if(optionsPreferences.getLevel() <= optionsPreferences.getCurrentLevel())
				optionsPreferences.setLevel(4);
			((Game)Gdx.app.getApplicationListener()).setScreen(new NextLevel(new grass3()));
		}
		...
		batch.begin();{		
			...
			batch.draw(arrowIn, 5, 360);
			batch.draw(arrowIn, 5, 232);
			batch.draw(arrowIn, 760, 360 + (32*3));
			batch.draw(arrowIn, 760, 232 - (32*3));
			...
			while(iter.hasNext()) {
				...
				z.changeToY(194, 232, Direction.DOWN);
				z.changeToX(194, 130, Direction.RIGHT);
				z.changeToY(354, 130, Direction.UP);
				z.changeToX(354, 232, Direction.RIGHT);
				z.changeToY(482, 232, Direction.UP);
				z.changeToX(482, 296, Direction.RIGHT);
				z.changeToY(642, 296, Direction.UP);
				z.changeToX(642, 456, Direction.RIGHT);
				...
				if(z.dx > 805 ) {
					...
					--playerLife;
					if(playerLife <= 0)
						((Game)Gdx.app.getApplicationListener()).setScreen(new GameOver(new sand3()));
				}
			}
			...
			while(iter1.hasNext()) {
				...
				z1.changeToY(194, 360, Direction.UP);
				z1.changeToX(194, 456, Direction.RIGHT);
				z1.changeToY(354, 456, Direction.DOWN);
				z1.changeToX(354, 360, Direction.RIGHT);
				z1.changeToY(482, 360, Direction.DOWN);
				z1.changeToX(482, 296, Direction.RIGHT);
				z1.changeToY(642, 296, Direction.DOWN);
				z1.changeToX(642, 130, Direction.RIGHT);
				z1.move(batch, delta);
				if(z1.health <= 0) {
					iter1.remove();	
