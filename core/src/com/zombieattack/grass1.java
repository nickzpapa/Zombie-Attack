...
public class grass1 implements Screen {
...
/**EDIT**/
		noClickZone = new NoClickZone("NoNoZones/level1.nono");
...
/**EDIT**/
	...
	public void render(float delta) {
		...
		//spawning zombies
		zombieList.spawnZombie(-20, 200, Direction.RIGHT);
		if(zombieList.isLevelFinished()){
			musicManager.stop();
			optionsPreferences.setCurrentLevel(7);
			if(optionsPreferences.getLevel() <= optionsPreferences.getCurrentLevel())
				optionsPreferences.setLevel(8);
			((Game)Gdx.app.getApplicationListener()).setScreen(new NextLevel(new water1()));
		}
...
/**EDIT**/		RegZombie z = (RegZombie) iter.next();
				z.changeToY(290, 200, Direction.UP);
				z.changeToX(290, 432, Direction.RIGHT);
				z.changeToY(610, 432, Direction.DOWN);
				z.changeToX(610, 200, Direction.RIGHT);
				z.move(batch, delta);
				if(z.health <= 0) {
...
/**EDIT**/		if(z.dx > 805 ) {
					...
					--playerLife;
					if(playerLife <= 0)
/**EDIT**/				((Game)Gdx.app.getApplicationListener()).setScreen(new GameOver(new grass1()));
				}
			}	
