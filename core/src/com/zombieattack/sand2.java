...
public class sand2 implements Screen {
...
/**EDIT**/
		noClickZone = new NoClickZone("NoNoZones/level8.nono");
		towerList = new TowerList(stage, zombieList, noClickZone);		

	
/**EDIT**/tiledMap = new TmxMapLoader().load("Maps/Level8.tmx");
...
/**EDIT**/
		...
		if(zombieList.isLevelFinished()){
			musicManager.stop();
			optionsPreferences.setCurrentLevel(2);
			if(optionsPreferences.getLevel() <= optionsPreferences.getCurrentLevel())
				optionsPreferences.setLevel(3);
			((Game)Gdx.app.getApplicationListener()).setScreen(new NextLevel(new grass1()));
		}
		...
		batch.begin();{		
			...
			while(iter.hasNext()) {
				...
				z.changeToY(98, 168, Direction.UP);
				z.changeToX(98, 464, Direction.RIGHT);
				z.changeToY(290, 464, Direction.DOWN);
				z.changeToX(290, 168, Direction.RIGHT);
				z.changeToY(482, 168, Direction.UP);
				z.changeToX(482, 464, Direction.RIGHT);
				z.changeToY(674, 464, Direction.DOWN);
				z.changeToX(674, 168, Direction.RIGHT);
				z.move(batch, delta);
				if(z.health <= 0) {
				...
				if(z.dx > 805 ) {
					...
					--playerLife;
					if(playerLife <= 0)
						((Game)Gdx.app.getApplicationListener()).setScreen(new GameOver(new sand2()));
				}
			}	
	...
	public void show() {	
		...
		zombieList = new ZombieList("SpawnTimes/level2.wave");
	}
