...
public class sand1 implements Screen {
...
/**EDIT**/
		noClickZone = new NoClickZone("NoNoZones/level7.nono");
		towerList = new TowerList(stage, zombieList, noClickZone);		

	
/**EDIT**/tiledMap = new TmxMapLoader().load("Maps/Level7.tmx");
...
/**EDIT**/
		if(zombieList.isLevelFinished()){
			optionsPreferences.setCurrentLevel(1);
			if(optionsPreferences.getLevel() <= optionsPreferences.getCurrentLevel())
				optionsPreferences.setLevel(2);
			musicManager.stop();
			((Game)Gdx.app.getApplicationListener()).setScreen(new NextLevel(new grass2()));
		}
		...
		batch.begin();{		
			...
			batch.draw(arrowIn, 5, 170);
			batch.draw(arrowOut, 5, 360);
			...
			while(iter.hasNext()) {
				...
				z.changeToY(130, 168, Direction.UP);
				z.changeToX(130, 272, Direction.RIGHT);
				z.changeToY(290, 272, Direction.DOWN);
				z.changeToX(290, 168, Direction.RIGHT);
				z.changeToY(514, 168, Direction.UP);
				z.changeToX(514, 272, Direction.RIGHT);
				z.changeToY(674, 272, Direction.UP);
				z.changeToX(674, 464, Direction.LEFT);
				z.changeToY(482, 464, Direction.DOWN);
				z.changeToX(482, 368, Direction.LEFT);
				z.changeToY(322, 368, Direction.UP);
				z.changeToX(322, 464, Direction.LEFT);
				z.changeToY(98, 464, Direction.DOWN);
				z.changeToX(98, 368, Direction.LEFT);
				z.move(batch, delta);
				if(z.health <= 0) {
				...
				if(z.dx < -20 && z.dy > 360 ) {
					...
					--playerLife;
					if(playerLife <= 0)
						((Game)Gdx.app.getApplicationListener()).setScreen(new GameOver(new sand1()));
				}
			}	
